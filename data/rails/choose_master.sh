...
do
  ...
  then
    echo "`ip_from_master_host` master" >> /etc/hosts
    break
  fi
done
...
do
  ...
  then
    ...
    then
      ...
      cat /etc/hosts > /tmp/hosts && sed -i "/\smaster/d" /tmp/hosts && cat /tmp/hosts > /etc/hosts && echo "$new_master_ip master" >> /etc/hosts
      exit 0
